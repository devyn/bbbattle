...
bbbout_stream *bbbout_open(char *path, uint16_t width, uint16_t height, unsigned char teams, struct rgb24 *team_colors) {
  ...
  fputs("bbbout1:", stream->out);

  check_endianness();

  /* dimensions */

  fput_be16(width,  stream->out);
  fput_be16(height, stream->out);

  /* teams */

  fputc('T', stream->out);

  fput_be16(teams, stream->out);

  int i;
  for (i = 1; i <= teams; i++) {
    fput_be16(i, stream->out);
    fwrite(&team_colors[i], sizeof(struct rgb24), 1, stream->out);
  }
...
int bbbout_write_cellset(bbbout_stream *stream, const bbbout_cellset cellset) {
  if (cellset.first == NULL) {
    fput_be16(0, stream->out);
  } else {
    bbbout_cellset_row *row = cellset.first;

    fput_be16(cellset.last->height + 1, stream->out);

    while (row != NULL) {
      fput_be16(row->y,    stream->out);
      fput_be16(row->size, stream->out);
      fwrite(row->points, sizeof(uint16_t), row->size, stream->out);
      row = row->next;
    }
  }
